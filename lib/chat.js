...
import { logChatBestEffort } from './chatLog.js';
...
export async function getChatReply(messages) {
  ...
  let contextText = await retrieveRelevantChunks(question);
  if (contextText === null) {
  ...
  const payload = {
    ...
    messages: [{ role: 'system', content: buildSystemPrompt(contextText) }, ...safeMessages],
    temperature: 0.4,
  ...
  await logChatBestEffort(question, reply);
